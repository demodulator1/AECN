...
class RNNModel(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=85, num_layers=3, dropout=0):
        """
        时间序列预测模型，使用RNN
        
        参数:
        input_dim: 每个时间步的特征维度 (6个值/列)
        hidden_dim: RNN的隐藏层维度
        num_layers: RNN层数
        dropout: dropout率
    ...
    def forward(self, x):
        ...
        # RNN处理
        rnn_out, _ = self.rnn(x)  # [batch, 4, hidden_dim]
        
        # 取最后一个时间步的输出
        rnn_out_last = rnn_out[:, -1, :]  # [batch, hidden_dim]
        
        # 输出层
        output = self.fc_out(rnn_out_last)  # [batch, 12]
