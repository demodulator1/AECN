...
class LSTMModel(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=16, num_layers=3, dropout=0.05):
    ...
    def forward(self, x):
        ...
        # 转换为LSTM输入
        x_lstm_input = self.fc_cnn(x_cnn_flat).unsqueeze(1)  # [batch, 1, hidden_dim]
        
        # LSTM处理
        x_lstm_output, _ = self.lstm(x_lstm_input)
        
        # 输出层
        output = self.fc_out(x_lstm_output.squeeze(1))  # [batch, 12]
