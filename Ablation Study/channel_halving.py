...
class CNNAttentionModel(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=6, num_heads=3, num_layers=3, dropout=0):
        ...
        # CNN层用于提取局部特征，通道数从16减少至8
        self.cnn = nn.Sequential(
            nn.Conv1d(input_dim, 8, kernel_size=3, padding=1),
            nn.GELU(),
            nn.Conv1d(8, 8, kernel_size=3, padding=1),
            nn.GELU()
        )
        
        # CNN输出的通道数是8，时间步长是4，展平后维度是8 * 4 = 32
        # 将CNN的输出转换为注意力层的输入
        self.fc_cnn = nn.Linear(32, hidden_dim)
    ...
    def forward(self, x):
        ...
        x_cnn = self.cnn(x_cnn)     # [batch, 8, 6]
