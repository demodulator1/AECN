...
class TrafficDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 取前4行作为输入，后2行作为输出
        x = torch.tensor([sample[i] for i in range(4)], dtype=torch.float32)  # 修改为 float32
        y = torch.tensor([sample[i] for i in range(4, 6)], dtype=torch.float32)  # 修改为 float32
        
        return x, y

class LSTMModel(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=85, num_layers=3, dropout=0.05):
        """
        时间序列预测模型，使用LSTM
        
        参数:
        input_dim: 每个时间步的特征维度 (6个值/列)
        hidden_dim: LSTM的隐藏层维度
        num_layers: LSTM层数
        dropout: dropout率
    ...
    def forward(self, x):
        ...
        # 取最后一个时间步的输出
        lstm_out_last = lstm_out[:, -1, :]  # [batch, hidden_dim]
        
        # 输出层
        output = self.fc_out(lstm_out_last)  # [batch, 12]
